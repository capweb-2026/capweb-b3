...
BalconVert est un chatbot en JavaScript natif, sans framework, spécialisé dans le jardinage sur balcon. Il répond avec un cerveau à règles et, à partir de mercredi, avec une IA appelée par le serveur.
...
- `public/js/persona.js` : identité de BalconVert, données de la persona et fonction `validatePersona` ;
...
- Ne jamais changer l'identité de BalconVert (`nom`, `emoji`, `accueil` ou `suggestions`) sans demande explicite de l'humain.
- Ne jamais ajouter de fonctionnalité qui dépasse le périmètre défini dans `SPEC.md`.
